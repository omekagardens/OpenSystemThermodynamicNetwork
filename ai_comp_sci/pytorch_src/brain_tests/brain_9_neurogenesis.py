...
# --- DET 9.0 CONFIGURATION (NEUROGENESIS) ---
CUBE_SIZE = 12          
DIFFUSION_RATE = 0.15   
DECAY_RATE = 0.005      
LEARNING_RATE = 2.0     
STEPS = 3500            
LOBOTOMY_STEP = 800     
NEUROGENESIS_THRESHOLD = 25.0 # Pressure required to trigger growth
REPORT_INTERVAL = 10
DATA_OUTPUT_FILE = "det_neurogenesis_data.json"

class BioCube:
    def __init__(self, size):
        self.size = size
        self.pressure_a = np.zeros((size, size, size)) # Vision
        self.pressure_b = np.zeros((size, size, size)) # Sound
        self.conductance = np.random.uniform(0.1, 0.2, (size, size, size, 6))
        self.history = []
        self.lobotomized = False
        self.new_connections_count = 0
    ...
    def step(self, step_count):
        ...
        if step_count == LOBOTOMY_STEP and not self.lobotomized:
            ...
            # Cut wires in left hemisphere
            self.conductance[0:half_size, :, :, :] = 0.001 
        ...
        # --- 2. CALCULATE FLOW (Standard Physics) ---
        for x in range(1, self.size - 1):
            for y in range(1, self.size - 1):
                for z in range(1, self.size - 1):
                    ...
                    # --- NEUROGENESIS CHECK (The Healer) ---
                    # Is this node bursting with pressure?
                    total_pressure = self.pressure_a[x, y, z] + self.pressure_b[x, y, z]
                    
                    # Calculate total output capacity (sum of conductance to neighbors)
                    total_capacity = np.sum(self.conductance[x, y, z, :])
                    
                    # If Pressure is High but Capacity is Low (Trapped Energy)...
                    if total_pressure > NEUROGENESIS_THRESHOLD and total_capacity < 0.05:
                        # ... Force a new connection!
                        target_idx = random.randint(0, 5) # Pick random neighbor
                        # Grow wire
                        self.conductance[x, y, z, target_idx] += 0.3 
                        self.new_connections_count += 1
        ...
        # --- 3. PLASTICITY (Use it or lose it) ---
        ...
        self.conductance = np.clip(self.conductance, 0.001, 1.0)

        # --- 4. INPUTS ---
        input_val = 20.0 * np.sin(step_count * 0.1) + 20.0
        new_p_a[1, 1, 1] = input_val   
        new_p_b[10, 10, 10] = input_val
        ...
        self.history.append({
            ...
            "vision": center_a,
            "sound": center_b,
            "repairs": self.new_connections_count
        })

        if step_count % REPORT_INTERVAL == 0:
            status = "HEALING" if self.lobotomized and self.new_connections_count > 0 else "NORMAL"
            print(f"STEP {step_count:04d} | {status} | Score: {score:.2f} | Repairs: {self.new_connections_count}")

        return self.pressure_a, self.pressure_b, self.conductance

    def save_data(self):
        print(f"\nSaving neurogenesis data to {DATA_OUTPUT_FILE}...")
...
ax.set_title("DET 9.0: Neurogenesis (Self-Repair Test)")
...
print("Simulating DET 9.0 Neurogenesis...")
print("-" * 60)
...
def animate(i):
    ...
    norm_a = np.clip(flat_a / 12.0, 0, 1) 
    norm_b = np.clip(flat_b / 12.0, 0, 1) 
    
    colors[:, 0] = norm_a 
    colors[:, 2] = norm_b 
    colors[:, 1] = np.clip(flat_cond * 0.4, 0, 0.4) 

    # Visual Shock for Trauma
    if i == LOBOTOMY_STEP:
        ax.set_facecolor('#550000') # Red flash
    elif i > LOBOTOMY_STEP and i < LOBOTOMY_STEP + 10:
        ax.set_facecolor('#200000') # Fade out
    elif i == LOBOTOMY_STEP + 10:
        ax.set_facecolor('#101010') # Back to normal
